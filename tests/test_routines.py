...
def write_wav(
    path: Path, seconds: float, rate: int = 16000, channels: int = 1, width: int = 2
) -> Path:
...
def test_demo_routine_loads_and_is_voice_ready():
    ...
    assert any(
        "say_pcm" in m or "tts" in m for m in moves
    ), "demo should include a voice line"
...
def test_dry_run_cli_exits_zero():
    r = subprocess.run(
        [sys.executable, str(REPO_ROOT / "examples" / "arm_dance.py"), "--dry-run"],
        capture_output=True,
        text=True,
        cwd=REPO_ROOT,
    )
...
def test_voice_check_cli(tmp_path):
    p = write_wav(tmp_path / "ok.wav", seconds=1.0)
    r = subprocess.run(
        [sys.executable, str(REPO_ROOT / "examples" / "voice.py"), "--check", str(p)],
        capture_output=True,
        text=True,
    )
...
def test_missing_iface_errors():
    r = subprocess.run(
        [sys.executable, str(REPO_ROOT / "examples" / "arm_dance.py")],
        capture_output=True,
        text=True,
        cwd=REPO_ROOT,
    )
